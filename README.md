...
`resourceRoots` tells UI5 that the `ui5x` namespace lives inside the application. Without it the
library is resolved against whichever resource root loaded the UI5 runtime, which is the
application itself in a standalone deployment but the shell in SAP Build Work Zone.

## Consuming UI5X in a SAP Fiori tools project

A UI5 library installed from npm is served under `/resources/`, the same path a SAP Fiori tools
project reserves for the SAPUI5 runtime. Every step below exists to stop one link in that chain
from intercepting the library before the application's own files are reached: the proxy during
local preview, the build, the deployment archive, the router in Cloud Foundry.

Installing the package and declaring the library are enough to use a control in a plain UI5
project. The rest apply as the application moves towards a deployment.

| Step | File | Needed for | Symptom when missing |
| ---- | ---- | ---------- | -------------------- |
| Declare the library | `manifest.json` | always | the control class cannot be loaded |
| Map the namespace | `manifest.json` | Work Zone | the library is requested from the shell instead of the application |
| Order the proxy | `ui5.yaml` | local preview | `404` on `/resources/ui5x/…` while `npm start` runs |
| Include in the build | `ui5.yaml` or the deploy configuration | any deployment | `dist` contains no `resources/ui5x` |
| Include in the archive | the `ui5-task-zipper` configuration | Cloud Foundry | the archive contains no `resources/ui5x` |
| Route the namespace | `xs-app.json` | HTML5 Repository | `ModuleError: failed to load 'ui5x/library.js' … script load error` |

### Local preview

Projects generated by SAP Fiori tools proxy `/resources` to `https://ui5.sap.com`. With the proxy
ahead of UI5 CLI's own middleware, requests for a locally installed library never reach the
dependency graph. Move it behind `serveResources`:

```yaml
server:
  customMiddleware:
    - name: fiori-tools-proxy
      afterMiddleware: serveResources     # instead of: compression
```

Projects providing a local UI5 configuration can use their `start-local` script instead.
...
UI5X ships a reusable UI5 build manifest, so UI5 CLI reuses the prebuilt library instead of
rebuilding its themes and preload in every consuming application. The build log reports this as
`Skipping build of library project ui5x`, which is the expected outcome.
...
```bash
unzip -l dist/<archive>.zip | grep "ui5x/library.js"
```

### Routing on Cloud Foundry

An application deployed to the HTML5 Application Repository routes `/resources/` to the SAPUI5
runtime, so a request for `/resources/ui5x/library.js` is answered by SAP's servers, where the
library does not exist. Add a route for the namespace **as the first entry** of `routes` in
`xs-app.json`, ahead of the one that forwards `/resources/`:

```json
{
  "source": "^/resources/ui5x/(.*)$",
  "target": "/resources/ui5x/$1",
  "service": "html5-apps-repo-rt",
  "authenticationType": "xsuaa"
}
```

Copy `service` and `authenticationType` from the catch-all route at the end of your own file if
they differ from these.

`xs-app.json` governs the routing of the whole application: a mistake there answers every request
with a `500`, not just the ones for this library. Keep a copy before editing it, check the result
with `python3 -m json.tool xs-app.json`, and confirm the application still starts before testing
the library.
...
```
https://<application-url>/resources/ui5x/library.js
```

A `404` means the request is being answered by the SAPUI5 runtime and the route is missing or
comes too late. Serving the file means the deployment is complete.
